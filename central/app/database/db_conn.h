...
public:
     ...
     */
    virtual int create_permission(const char *set_name, const char *entity,
            const unsigned int entity_type, const unsigned int op) const = 0;

    /* 
     * Update the permission for the selected set.
     */
    virtual int update_permission(const char *set_name, const char *entity,
            const unsigned int op) const = 0;

    /* 
     * Delete the permission with the given attributes.
     * (set, entity) should be a unique identified for a permission.
     */
    virtual int delete_permission(const char *set_name, 
